...
def compare_proteoms_info():
    ...
    info_text_rus = {'Main': 'Данный инструмент позволяет найти общие моты в белках двух протеомов',
                     'FROM proteom path': 'Исследуемый протеом. Для поиска выберите протеом из списка предложенных в директории /data/proteoms или укажите путь до файла протеома в формате .fasta, нажав кнопку Browse.',
                     'TO proteom path': 'Сравниваемый протеом. Для поиска выберите протеом из списка предложенных в директории /data/proteoms или укажите путь до файла протеома в формате .fasta, нажав кнопку Browse.',
                     ...
                     'Search way': 'Программа может осуществлять два варианта поиска мотов:\n'
                                   ...
                                   'Подходит для поиска всех вохможных мотов заданной длины, исключает перекрытие'
...
def align_seqs_info():
    import PySimpleGUI as sg
    info_text_rus = {'main':'Этот инструмент позволяет осуществить простое выравнивание двух белков на основании общего участка',
    'info':'Для выполнения программы введите название белков и первичную последовательность в соответствющие поля. На данный момент программа совершает простое выравнивание двух белков'
